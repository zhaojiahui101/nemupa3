...
static int cmd_bt(char *args) {
	...
	int i = 0;
	while(ebp != 0) {
		sf.args[0] = swaddr_read(ebp + 8, 4, S_SS);
		sf.args[1] = swaddr_read(ebp + 12, 4, S_SS);
		sf.args[2] = swaddr_read(ebp + 16, 4, S_SS);
		sf.args[3] = swaddr_read(ebp + 20, 4, S_SS);

		printf("#%d 0x%08x in %s (0x%08x 0x%08x 0x%08x 0x%08x)\n", i, eip, find_fun_name(eip), sf.args[0], sf.args[1], sf.args[2], sf.args[3]);
		i ++;
		eip = swaddr_read(ebp + 4, 4, S_SS);
